import { Component, OnInit, ViewChild } from '@angular/core';
import { MatDialog } from '@angular/material/dialog';
import { MatPaginator } from '@angular/material/paginator';
import { MatSort } from '@angular/material/sort';
import { MatTableDataSource } from '@angular/material/table';
import * as moment from 'moment';
import { RiderService } from 'src/app/shared/services/rider.service';
import { ToastService } from 'src/app/shared/services/toast.service';
import { PayoutsDialogComponent } from '../../admin-panel/payouts/payouts-dialog/payouts-dialog.component';
import { DeliveryStatusList, RiderOrder } from '../rider-orders/model/rider-order';
import { RiderPayout, FilterRiderPayout, RiderPayoutAction, RiderPayoutStatusList, RiderShiftPayout } from './model/rider-payout';
import pdfmake from 'pdfmake/build/pdfmake';
import pdfFonts from 'pdfmake/build/vfs_fonts';
import { Router } from '@angular/router';
import { dateFormat, dateLongTimeFormat, pageSize, pageSizeOptions } from 'src/app/shared/models/constants/constant.type';
import { downloadFile } from 'src/app/shared/functions/modular.functions';
import { validateDate } from 'src/app/shared/functions/common-validation.functions';
pdfmake.vfs = pdfFonts.pdfMake.vfs;
@Component({
  selector: 'app-rider-payouts',
  templateUrl: './rider-payouts.component.html',
  styleUrls: ['./rider-payouts.component.scss']
})
export class RiderPayoutsComponent implements OnInit {
  payoutsList: MatTableDataSource<RiderPayout> = new MatTableDataSource();
  displayedColumns: string[] = ['payoutId', 'riderName', 'startDate', 'endDate', 'amountPaidToVendor', 'payoutCompletedTime', 'payoutStatus', 'action'];
  pageIndex: number = 0;
  pageSize = pageSize;
  pageSizeOptions = pageSizeOptions;
  totalPayoutRecords: number;
  showFilterFields: boolean;
  filterRiderPayoutFields: FilterRiderPayout = new FilterRiderPayout();
  readonly payoutStatusList = RiderPayoutStatusList;
  readonly deliveryStatusList = DeliveryStatusList
  readonly dateFormat = dateFormat;
  readonly dateLongTimeFormat = dateLongTimeFormat;
  @ViewChild(MatPaginator) paginator: MatPaginator;
  @ViewChild(MatSort) sort: MatSort;
  constructor(private riderService: RiderService, private dialog: MatDialog, private toastMsgService: ToastService,
    private router: Router) { }
  ngOnInit(): void {
    this.getPayoutsData();
  }
  getPayoutsData(filterFlag?: boolean) {
    if (!this.isValidFilterFields()) return;
    if (filterFlag) {
      this.pageIndex = 0;
      this.paginator.pageIndex = 0
    }
    this.filterRiderPayoutFields.pageIndex = this.pageIndex;
    this.filterRiderPayoutFields.pageSize = this.pageSize;
    const data = this.filterRiderPayoutFields.toJson();
    this.riderService.filterRiderPayouts(data).subscribe(res => {
      this.payoutsList.data = [];
      this.totalPayoutRecords = res['result']['total_records'];
      for (const i of res['result']['payouts']) {
        this.payoutsList.data.push(RiderPayout.fromJson(i));
      }
      this.payoutsList.sort = this.sort;
    })
  }
  /**
   * Method that make func call based on the action it recevied
   * @param action 
   * @param payout 
   * @returns 
   */
  takeAction(action: RiderPayoutAction, payout: RiderPayout) {
    if (action === RiderPayoutAction.Retry) return this.retryPayout(payout);
    if (action === RiderPayoutAction.StopRetry) return this.stopRetryingPayout(payout);
    if (action === RiderPayoutAction.MarkComplete) return this.markComplete(payout);
    if (action === RiderPayoutAction.ExportPDF) return this.generatePdf(payout);
  }
  /**
   * Method that retry payment for the payout
   * @param payout 
   */
  retryPayout(payout: RiderPayout) {
    this.riderService.retryRiderPayout(payout.payoutId).subscribe(res => {
      this.toastMsgService.showSuccess(`Retry for ID: ${payout.payoutId} done successfully`);
      this.getPayoutsData(false);
    })
  }
  /**
   * Method that stops retrying for the payout
   * @param payout 
   */
  stopRetryingPayout(payout: RiderPayout) {
    this.riderService.stopRetryRiderPayout(payout.payoutId).subscribe(res => {
      this.toastMsgService.showSuccess(`Stopped retry for ID: ${payout.payoutId}`);
      this.getPayoutsData(false);
    })
  }
  /**
   * Method that manually marks the payout as complete
   * @param payout 
   */
  markComplete(payout: RiderPayout) {
    const dialogRef = this.dialog.open(PayoutsDialogComponent, {
      data: {
        action: 'mark-complete'
      },
      disableClose: true
    })
    dialogRef.afterClosed().subscribe(response => {
      if (response.flag) {
        const data = {
          transaction_details: {
            transaction_id: response.txnId
          },
          payout_completed_time: response.paymentDate
        }
        this.riderService.markAsCompleteRiderPayout(payout.payoutId, data).subscribe(res => {
          this.toastMsgService.showSuccess(`ID: ${payout.payoutId} is marked complete`);
          this.getPayoutsData();
        })
      }
    })
  }
  /**
  * Method that exports payout report in pdf
  * @param payout 
  */
  public generatePdf(payout: RiderPayout) {
    const layout = {
      fillColor: (i, node) => {
        return (i % 2 === 0) ? '#fafafe' : null;
      },
      hLineColor: (i, node) => {
        return (i === 0 || i === node.table.body.length) ? 'black' : 'gray';
      },
      vLineColor: (i, node) => {
        return (i === 0 || i === node.table.widths.length) ? 'black' : 'gray';
      }
    }
    const pdfData = {
      background: (currentPage, pageSize) => {
        return {
          image: 'data:image/png;base64,',
          height: 200,
          opacity: 0.2,
          margin: [(pageSize.width - 200) / 2, (pageSize.height - 200) / 2, 0, 0]
        }
      },
      content: this.addContentInPDF(payout, layout),
      styles: {
        header: {
          fontSize: 18,
          alignment: 'center',
          bold: true,
          margin: [0, 0, 0, 10]
        },
        subHeader: {
          fontSize: 14,
          bold: true,
          margin: [0, 0, 0, 20]
        },
        tableHeader: {
          bold: true,
          fontSize: 13,
          color: 'black'
        },
        shiftHeader: {
          fontSize: 16,
          margin: [0, 10, 0, 10],
          bold: true
        }
      },
    };
    pdfmake.createPdf(pdfData).download(`${payout.riderName}_${payout.payoutId}.pdf`);
  }

  /**
   * Method that adds content in pdf
   * @param payout 
   * @param layout 
   * @returns 
   */
  addContentInPDF(payout: RiderPayout, layout: any) {
    let content = [
      { text: `Rider Payout Report`, style: 'header' },
      { text: [{ text: 'Payout ID: ', style: 'subHeader' }, payout.payoutId] },
      { text: [{ text: 'Rider Name: ', style: 'subHeader' }, payout.riderName] },
      { text: [{ text: 'Duration: ', style: 'subHeader' }, `${moment(payout.startDate).format('DD/MM/YYYY')} - ${moment(payout.endDate).format('DD/MM/YYYY')}`] },
      { text: [{ text: 'Status: ', style: 'subHeader' }, payout.payoutStatus] },
      { text: [{ text: 'Processed On: ', style: 'subHeader' }, payout.payoutCompletedTime ? moment(payout.payoutCompletedTime).format('DD/MM/YYYY') : 'N/A'] },
      { text: `Shiftwise details`, style: 'shiftHeader' },
      this.addShiftPayoutsInPDF(payout.shiftPayouts, layout),
      {
        margin: [0, 10, 0, 10],
        layout,
        table: {
          widths: ['50%', '50%'],
          body: [
            ['Total Order Amount', payout.totalOrderAmount],
            ['Transaction Changes', payout.txnCharges],
            ['Amount Paid', payout.amountPaidToRider]
          ]
        }
      },
    ]
    return content;
  }

  /**
   * Method that adds shiftwise payout details in pdf
   * @param shiftPayouts 
   * @param layout 
   * @returns 
   */
  addShiftPayoutsInPDF(shiftPayouts: RiderShiftPayout[], layout: any) {
    let body = [];
    for (const i of shiftPayouts) {
      body.push(
        { text: [{ text: 'Rider Shift ID: ', style: 'subHeader' }, i['riderShiftId']] },
        { text: [{ text: 'Shift Name: ', style: 'subHeader' }, i['shiftName']] },
        { text: [{ text: 'Date: ', style: 'subHeader' }, i['firstPingAt']] },
        {
          margin: [0, 0, 0, 10],
          layout,
          table: {
            headerRows: 1,
            body: this.addPayoutOrdersInPDF(i.payoutOrders)
          }
        },
      );
    }
    return body;
  }
  /**
   * Method that adds payout orders data in pdf
   * @param payout 
   * @returns 
   */
  addPayoutOrdersInPDF(payoutOrders: RiderOrder[]) {
    let body = [];
    body.push([
      { text: 'Order ID', style: 'tableHeader' },
      { text: 'Client ID', style: 'tableHeader' },
      { text: 'Client Order ID', style: 'tableHeader' },
      { text: 'Rider Payout', style: 'tableHeader' },
      { text: 'Actual Rider Payout', style: 'tableHeader' },
      { text: 'Remarks (if any)', style: 'tableHeader' }
    ])
    for (const i of payoutOrders) {
      let actualRiderPayout, remarks
      if (i['deliveryStatus'] === this.deliveryStatusList.CANCELLED) {
        actualRiderPayout = i['riderPayoutSettlementAmt'];
        remarks = i['cancellationReason'];
      } else {
        actualRiderPayout = i['riderPayoutAmt'];
        remarks = '';
      }
      body.push([i['orderId'], i['clientId'], i['clientOrderId'], i['riderPayoutAmt'], actualRiderPayout, remarks])
    }
    return body;
  }

  /**
   * Method that navigates to rider-orders page to show orders with that payoutId
   * @param payoutId 
   */
  navigateToRiderOrdersPage(payoutId: string) {
    this.router.navigate(['rider/orders'], { queryParams: { payoutId } })
  }
  /**
   * Method that invokes on page change
   * @param event 
   */
  onPageChange(event) {
    this.pageIndex = event.pageIndex;
    this.pageSize = event.pageSize;
    this.getPayoutsData();
  }
  /**
   * Method that reset filter params
   * @param fieldName 
   */
  clearFilter(fieldName: 'all' | 'startDate' | 'endDate') {
    if (fieldName === 'all') {
      this.showFilterFields = false;
      this.filterRiderPayoutFields = new FilterRiderPayout();
    } 
    else if (fieldName === 'startDate') this.filterRiderPayoutFields.startDate = null;
    else if (fieldName === 'endDate') this.filterRiderPayoutFields.endDate = null;
    this.getPayoutsData();
  }

  /**
   * method that checks the validity of the filter fields
   * @returns 
   */
   isValidFilterFields(): boolean {
    const validationItems = [
      {validator: validateDate, args: [this.filterRiderPayoutFields.startDate, this.filterRiderPayoutFields.endDate]},
      {validator: validateDate, args: [this.filterRiderPayoutFields.processedOnStartDate, this.filterRiderPayoutFields.processedOnEndDate]},
    ]
    for (const item of validationItems) {
      const errorMsg = item.validator.apply(null, item.args);
      if (errorMsg) {
        this.toastMsgService.showError(errorMsg);
        return false;
      }
    }
    return true
  }

   /**
   * Method to download filtered rider payouts in csv
   */
   exportRiderPayoutsInCsv() {
    const data = this.filterRiderPayoutFields.toJson();
    data['filter']['in_csv'] = true;
  
    this.riderService.downloadCsvFile('riderPayouts', data).subscribe(
      res => {
        downloadFile(res, 'rider payouts');
      });
  }

}
